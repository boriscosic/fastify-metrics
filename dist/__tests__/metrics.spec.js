...
(0, globals_1.describe)('metrics plugin', () => {
    ...
    (0, globals_1.test)('should register default metrics for 4xx request', async () => {
        await app.inject({
            method: 'GET',
            url: '/not-exists',
        });
        ...
        (0, globals_1.expect)(metrics.payload).toContain('# HELP http_request_duration_seconds request duration in seconds');
        (0, globals_1.expect)(metrics.payload).toContain('# TYPE http_request_duration_seconds histogram');
        (0, globals_1.expect)(metrics.payload).toContain('http_request_duration_seconds_bucket{le="0.05",method="GET",route="/not-exists",status_code="404"}');
        (0, globals_1.expect)(metrics.payload).toContain('http_request_duration_seconds_sum{method="GET",route="/not-exists",status_code="404"}');
        (0, globals_1.expect)(metrics.payload).toContain('http_request_duration_seconds_count{method="GET",route="/not-exists",status_code="404"}');
        (0, globals_1.expect)(metrics.payload).toContain('# HELP http_request_summary_seconds request duration in seconds summary');
        (0, globals_1.expect)(metrics.payload).toContain('# TYPE http_request_summary_seconds summary');
        (0, globals_1.expect)(metrics.payload).toContain('http_request_summary_seconds{quantile="0.5",method="GET",route="/not-exists",status_code="404"');
        (0, globals_1.expect)(metrics.payload).toContain('http_request_summary_seconds{quantile="0.5",method="GET",route="/not-exists",status_code="404"');
    });
});
